...
// ?========================== passport serialzie and deserialize
/*
 todo: Passport "serializes" objects to make them easy to store, converting the
 todo: user to an identifier (id) >>> turn object into JSON to send.
 */
...
passport.deserializeUser(function(id, cb) {
  db.user.findByPk(id).then(function(user) {
    cb(null, user);
  }).catch(cb); //! if get error, try to send back user again
});
